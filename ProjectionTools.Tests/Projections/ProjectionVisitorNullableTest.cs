...
[TestFixture]
public class ProjectionVisitorNullableTest
{
    private static readonly ProjectionVisitor Visitor = new();

    private static Projection<string, int?> CreateProjection => new(s => s != null ? s.Length : null);
    ...
    public void Projection__Project_invocation(ProjectionSource source)
    {
        ...
        Assert.That(result.ToString(), Is.EqualTo(@"x => IIF((x != null), Convert(x.Length, Nullable`1), null)"));
    ...
    public void Projection__Project_reference(ProjectionSource source)
    {
        ...
        Assert.That(result.ToString(), Is.EqualTo(@"x => x.Select(s => IIF((s != null), Convert(s.Length, Nullable`1), null)).First()"));
